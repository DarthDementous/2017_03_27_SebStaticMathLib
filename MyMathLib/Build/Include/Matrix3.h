...
public:
	///Constructors
	LIBRARY_API Matrix3() : m11(1), m12(0), m13(0),                                                         //Identity matrix by default
		        m21(0), m22(1), m23(0),
		        m31(0), m32(0), m33(1) {}

	LIBRARY_API Matrix3(Matrix3 &a_rhs) : m11(a_rhs.m11), m12(a_rhs.m12), m13(a_rhs.m13),                 //Copy constructor, copy information from Matrix into the other
						      m21(a_rhs.m21), m22(a_rhs.m22), m23(a_rhs.m23),
							  m31(a_rhs.m31), m32(a_rhs.m32), m33(a_rhs.m33) {}             

	LIBRARY_API Matrix3(float *a_ptr) : m11(a_ptr[0]), m12(a_ptr[1]), m13(a_ptr[2]),                                          //Construct from list of floats
		                    m21(a_ptr[3]), m22(a_ptr[4]), m23(a_ptr[5]),
		                    m31(a_ptr[6]), m32(a_ptr[7]), m33(a_ptr[8]) {}

	LIBRARY_API Matrix3(T a_m11, T a_m12, T a_m13,
...
#pragma region Base Operators
	LIBRARY_API Matrix3 operator * (const Matrix3 &a_rhs) const {
		/*
	...
	LIBRARY_API Vector3<T> operator * (const Vector3<T> &a_rhs) const {                      /*Because we're not in the Vector2 template class, we need to specify what type it is in all uses.
	...
	LIBRARY_API Matrix3 operator = (const Matrix3 &a_rhs) {
...
#pragma region Functions
	///Creators
	LIBRARY_API static Matrix3 createIdentity() {                                      //Return default matrix
		return Matrix3<T>(1, 0, 0
			              0, 1, 0
			              0, 0, 1);
	}

	LIBRARY_API static Matrix3 createRotationX(float a_rot) {                          //Return X rotated Matrix3
		Matrix3<T> tmp;
	...
	LIBRARY_API static Matrix3 createRotationY(float a_rot) {
		Matrix3<T> tmp;
	...
	LIBRARY_API static Matrix3 createRotationZ(float a_rot) {
		Matrix3<T> tmp;
		tmp.mm[0][0] = cosf(a_rot);   //Xx
	...
	LIBRARY_API static Matrix3 createScale(float a_xScale, float a_yScale, float a_zScale) {           //Return scaled Matrix2
		Matrix3<T> tmp;
	...
	LIBRARY_API void setRotateX(float a_rot) {                          //Rotate Matrix3 around X axis
	...
	/// Getters
	LIBRARY_API Vector3<T> getTranslation() {
	...
	LIBRARY_API float getRotation(char a_axis) {
		switch (a_axis)
		{
...
#pragma endregion
	...
	LIBRARY_API Vector3<T> & operator [] (unsigned int index) { return v[index]; } //Return vector at specified index
};
