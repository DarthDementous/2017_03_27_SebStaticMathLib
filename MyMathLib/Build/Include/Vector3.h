...
public:
	///Constructor
	LIBRARY_API Vector3() : x(0), y(0), z(0) {}
	LIBRARY_API Vector3(T a_x, T a_y, T a_z) : x(a_x), y(a_y), z(a_z) {}
	LIBRARY_API Vector3(const Vector3& a_rhs) : x(a_rhs.x), y(a_rhs.y), z(a_rhs.z) {}   //Copy/reference constructor
...
#pragma region Base Operators
	LIBRARY_API Vector3 operator + (const Vector3& a_rhs) const                       // V3 = V3 + V3 
	{
	...
	LIBRARY_API Vector3& operator = (const Vector3& a_rhs)							// V3 = V3
	{
	...
	LIBRARY_API Vector3 operator * (S a_scalar) const								// V3 * f
	{
	...
	LIBRARY_API friend Vector3 operator * (S a_scalar, const Vector3& a_rhs)		//  f * V3
	{
	...
	LIBRARY_API Vector3 operator -() const									// -V3
	{
		return Vector3<T>(-x, -y, -z);                          // Return negative version of vector3
	}
	///Conditions
	LIBRARY_API bool operator == (const Vector3 &a_rhs) const				// V3 == V3
	{
...
#pragma region Advanced Operators
	...
	template <typename S>
	LIBRARY_API Vector3& operator *= (S a_scalar)									// V3 *= f
	{
		*this = *this * a_scalar;
		return *this;
	}
...
#pragma region Functions
	LIBRARY_API float dot(const Vector3& a_rhs)										// dot product, checks if vectors cast a shadow
	{
		return ((x * a_rhs.x) + (y * a_rhs.y) + (z * a_rhs.z));
	}

	LIBRARY_API Vector3 cross(const Vector3& a_rhs) {                               // Returns vector perpendicular to the two vectors
		/*
		x   a_x         x | y z x y | z
		y   a_y         x | y z x y | z
		z   a_z
	...
	LIBRARY_API Vector3 getNormal() {
		float m = magnitude();
		return Vector3<T>(x /= m, y /= m, z /= m);                       //Return normalized version of lhs
	}
#pragma endregion
	//Casts vector to float
	LIBRARY_API explicit operator float* () { return &x; }								// * Return start of memory address for Vector3
